...
if os.path.isfile(os.path.join(BACKUP_DIR, backup_file)):
    ...
    try:
        ...
        s3_client = boto3.client("s3", config=botocore.config.Config(verify=False))  # Disable SSL verification
        ...
        sns_client.publish(
            TopicArn=SNS_ARN,
            Subject=EMAIL_SUBJECT,
            Message="Tableau UAT Server backup failed. Error: {}".format(str(e))
        )
else:
